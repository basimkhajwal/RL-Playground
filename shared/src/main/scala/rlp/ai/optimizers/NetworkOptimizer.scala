...
class SGDMomentum(network: NeuralNetwork,
  ...
  val numWeights = velocity.length
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    val networkA, networkB = new NeuralNetwork(Array(2, 5, 1), Array(Sigmoid, ReLU))

    networkA.randomiseWeights()
    for (i <- networkA.weights.indices) networkB.weights(i) = new Matrix(networkA.weights(i))
    ...
    println("Epoch\t\tSGD\t\t\t\tSGD+Momentum")
    for (i <- 0 until 200) {
